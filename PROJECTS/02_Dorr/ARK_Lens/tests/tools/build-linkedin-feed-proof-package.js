...
const ENTRIES = Object.freeze([
  ["proofs/linkedin_feed/manifest.json", "manifest.json"],
...
function copyExact(sourceRelative, targetRelative) {
  ...
  if (!fs.statSync(source).isFile()) throw new Error(`Feed proof entry must be an exact file: ${sourceRelative}`);
...
packaged.forEach((relativePath) => {
  if (!allowed.has(relativePath)) throw new Error(`Feed proof file is outside the exact allow-list: ${relativePath}`);
